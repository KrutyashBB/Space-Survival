...
public class ShipStoragePanel : Sprite
{
    ...
    public ShipStoragePanel(Texture2D tex, Vector2 pos, float scale) : base(tex, pos, scale)
    {
        for (var i = 0; i < CountCells; i++)
            Cells.Add(new CellInventoryPanel(CellOwner.ShipStorage, _cellTexture, new Vector2(0, 0), CellScale));
    }
